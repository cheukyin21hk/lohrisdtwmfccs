public abstract class FourierTransform {
	...
	protected void hamming(float[] samples) {
		for (int i = 0; i < samples.length; i++) {
			samples[i] *= (0.54f - 0.46f * Math.cos(TWO_PI * i
					/ (samples.length - 1)));
		}
	}
	 ...
	 *            timeSize() samples between the starting index and the end of
	 *            the buffer. If there aren't, an error will be issued and the
